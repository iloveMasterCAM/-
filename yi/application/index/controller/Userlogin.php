...
class userlogin extends Controller{
 ...
 public function wblogin(){
  ...
  $wb = new SaeTOAuthV2(WB_AKEY,WB_SKEY);
        ...
        if($token){
               $wbo = new SaeTClientV2(WB_AKEY,WB_SKEY,$token['access_token']);
            $vl = $wbo->get_uid();
           $message =  $wbo->show_user_by_id($vl['uid']);
            $user = Db::name('user');
...
$user->insert(['id'=>null,'account'=>$token['uid'],'nickname'=>$message['screen_name'],'header'=>$message['profile_image_url'],'pasw'=>'123456','token'=>$token['access_token'],'expire_time'=>$t,'skin'=>0,'Recommend'=>0,'history'=>0,'remind'=>0,'color'=>0]);
